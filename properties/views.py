...
# function takes string string string string (whichform, firstname, lastname, email)->int (send mail to tumuratas)
from pages.views import send_mail_to_tumuratas
...
def property_detail(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            form.save()
            send_mail_to_tumuratas('Interesse an einer Immobilie',
                                   request.POST['firstname'], request.POST['lastname'], request.POST['email'], "Interesse an einer Immobilie", request.POST['tel'])
...
def immoAnbieten(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            offer = form.save()
            send_mail_to_tumuratas(
                'Immobilie Anbieten Formular', request.POST['firstname'], request.POST['lastname'], request.POST['email'], "Nachricht wurde vollständig übertragen.", request.POST['tel'], objecttype=request.POST['objecttype'], market_plan=request.POST['market_plan'], total_size=request.POST['total_size'], street=request.POST['street'], building_year=request.POST['building_year'], ground_size=request.POST['ground_size'], living_size=request.POST['living_size'], housenumber=request.POST['housenumber'], postialcode=request.POST['postialcode'], city=request.POST['city'], country=request.POST['country'], notification=request.POST['notification'])
            return redirect('pages:anfrageerhalten')
        else:
            #errors = form.errors
            form = PropertyProviderForm(
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not captcha:
                ...
                """
            form.save()
            send_mail_to_tumuratas(request.POST['firstname'], request.POST['lastname'], request.POST['email'], request.POST['tel'],
                                   request.POST['search_for_objecttype'], request.POST['searched_city'], request.POST['searched_postialcode'], request.POST['distance'])
            return redirect('pages:anfrageerhalten')
        else:
            print("not valid")
            print(form.errors)
